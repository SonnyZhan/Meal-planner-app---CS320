...
def get_meal_menu(location, date=datetime.date.today()):
    # Get the full menu for the specified location and date
    menu = get_menu(location, date)
    
    # Filter the menu to include only breakfast items
    breakfast_items = [item for item in menu if item.get("meal-name") == "breakfast"]
